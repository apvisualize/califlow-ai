...
function onResults(results) {
    ...
    drawConnectors(canvasCtx, lm, POSE_CONNECTIONS, {color: '#ffffff', lineWidth: 4});
    drawLandmarks(canvasCtx, lm, {color: isBackStraight ? '#00f3ff' : '#ff0055', radius: 5});
    
    // --- LOGIKA REPETISI (PUSH / PULL / SQUAT) ---
    if (activeWorkout.type === 'push' || activeWorkout.type === 'pull' || activeWorkout.type === 'squat') {
        
        let currentAngle = (activeWorkout.type === 'squat') ? calculateAngle(side.h, side.k, lm[leftVisible ? 27 : 28]) : angle;
        ...
        if (activeWorkout.type === 'pull') {
            ...
            if (currentAngle > startThreshold && (isHandsUp || exId === 'australian_pullup')) {
                stage = 'hanging'; 
            }
    ...
    // --- LOGIKA HOLD ---
        ...
        if (exId === 'dead_hang') {
            ...
            if (stage === 'at_top' && !isBottomPosition) {
                isPoseCorrect = true; 
                eccentricDuration = Math.floor((Date.now() - eccentricStartTime) / 1000);
            }
            ...
            if (isElbowBent && isBackStraight && isHorizontal) {
                ...
                poseFeedback = !isHorizontal ? "GET LOWER (HORIZONTAL)!" : (!isElbowBent ? "CHECK ELBOWS (90°)!" : "FIX HIPS!");
            }
        }
        else {
        ...
        if (isPoseCorrect) {
            ...
            let duration = (exId === 'negative_pullup') ? eccentricDuration : Math.floor((Date.now() - holdStartTime) / 1000);
            updateUI(repDisplay, feedback, duration, "HOLDING... ⏳");
            feedback.style.borderColor = "rgba(255, 255, 255, 0.15)";
            feedback.style.color = "white";
            
            if (duration > 0 && duration % 5 === 0 && duration !== holdDuration) speak(duration + " seconds");
            holdDuration = duration;

            if (exId !== 'negative_pullup' && duration >= activeWorkout.target) finishExam();
        } else {
    ...
    if ((activeWorkout.type !== 'hold' || exId === 'negative_pullup') && reps >= activeWorkout.target) {
        finishExam();
    }
...
function finishExam() {
    ...
    summaryOverlay.style = "position:fixed;inset:0;background:rgba(0,0,0,0.95);display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:9999;color:white;backdrop-filter:blur(10px);";
...
function initApp() {
    ...
    startOverlay.style = "position:fixed;inset:0;background:black;z-index:10000;display:flex;flex-direction:column;align-items:center;justify-content:center;";
    ...
    document.getElementById('start-btn').onclick = () => {
        ...
        sfxRep.play().then(() => { sfxRep.pause(); }).catch(e => console.log(e));
        startOverlay.remove();
