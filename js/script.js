...
function init() {
    ...
    roadmaps.forEach(roadmap => {
        const completedSteps = roadmap.steps.filter(stepId => userProgress.includes(stepId)).length;
        const progressPercent = Math.round((completedSteps / roadmap.steps.length) * 100);
...
window.init = init;
window.closeModal = () => document.getElementById('modal-simulation').classList.add('hidden');
window.launchCamera = () => {
    if (activeWorkoutId) {
...
function showSteps(roadmapId) {
    ...
    roadmapSteps.forEach(workout => {
        ...
        const isUnlocked = (workout.req || []).every(r => userProgress.includes(r));
...
function createCard(workout, isUnlocked, isCompleted) {
    ...
    `;
    card.onclick = () => (isUnlocked || isCompleted) ? openWorkout(workout.id) : alert("Complete previous steps first!");
    grid.appendChild(card);
}
